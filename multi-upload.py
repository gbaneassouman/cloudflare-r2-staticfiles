...
def upload_directory(dir_path: Path):
    """Upload récursif d'un dossier (avec multithreading + barre de progression)."""
    all_files = []
    ...
    if not all_files:
        # Dossier vide → créer un objet "dossier/"
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        with tqdm(total=len(all_files), desc="📤 Upload en cours", unit="fichier") as pbar:
            for future in as_completed(futures):
                future.result()  # on pourrait vérifier True/False ici
                pbar.update(1)
